...
def main(args):
    ...
    for detector in detectors:
        for variable in variables:
            for quantile in quantiles:
                name = '_'.join(['mc', 'weights', detector, variable, str(quantile).replace('.', 'p')]) + '.pkl'
                mc_outputs.append(name)
    for detector in detectors:
        for variable in variables_iso:
            for quantile in quantiles:
                name = '_'.join(['mc', 'weights', 'tail', detector, variable, str(quantile).replace('.', 'p')]) + '.pkl'
                mc_outputs.append(name)
    mc_outputs = mc_outputs + ['mc_clf_3Cat_EB_probeChIso03_probeChIso03worst.pkl', 'mc_clf_3Cat_EE_probeChIso03_probeChIso03worst.pkl', 'mc_clf_p0t_EB_probePhoIso.pkl', 'mc_clf_p0t_EE_probePhoIso.pkl']
    ...
    mc_missing = []
    for name in mc_outputs:
        if name not in os.listdir(directory):
            mc_missing.append(name)
    for name in mc_missing:
        print(name)
    print('Missing {} MC models'.format(len(mc_missing)))
