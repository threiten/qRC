...
cd /mnt/t3nfs01/data01/shome/threiten/QReg/qRC/scripts/
...
do
	...
	qsub -q all.q -l h_vmem=6G job_train_qRC_data.sh ${configSS} ${var} ${q} ${N_evts} ${EBEE} ${spl}
    done
done
...
do
    ...
    do
	echo Submitting training job for quantile $q on data
	qsub -q all.q -l h_vmem=6G job_train_qRC_I_data.sh ${configPI} ${var} ${q} ${N_evts} ${EBEE} ${spl}
    done
done
...
do
    ...
    do
	echo Submitting training job for quantile $q on data
	qsub -q all.q -l h_vmem=6G job_train_qRC_I_data.sh ${configCI} ${var} ${q} ${N_evts} ${EBEE} ${spl}
    done
done